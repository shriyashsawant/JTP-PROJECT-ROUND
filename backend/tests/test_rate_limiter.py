...
class TestRateLimiterTokenConsumption:
    async def test_allows_requests_up_to_capacity(self):
        ...
        assert await limiter.try_consume() is True
        assert await limiter.try_consume() is True
        assert await limiter.try_consume() is True
    ...
    async def test_rejects_once_capacity_is_exhausted(self):
        ...
        assert await limiter.try_consume() is True
        assert await limiter.try_consume() is True
        assert await limiter.try_consume() is False
...
class TestCheckRateLimit:
    async def test_different_keys_get_independent_buckets(self):
        ...
        assert await check_rate_limit(key_a, requests_per_minute=5) is False
        assert await check_rate_limit(key_b, requests_per_minute=5) is True

    async def test_same_key_reuses_the_same_bucket(self):
        key = _unique_key("reuse")
        for _ in range(3):
            assert await check_rate_limit(key, requests_per_minute=3) is True
        assert await check_rate_limit(key, requests_per_minute=3) is False
