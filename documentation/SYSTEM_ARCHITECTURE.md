...
*   **Presentation Layer (Next.js)**: A React-based web client running on Next.js 16 (App Router), employing TypeScript and dynamic styling to handle user queries, result visualization, and scent profile analysis. See [FRONTEND_ARCHITECTURE.md](file:///c:/Users/SHRIYASH%20SAWANT/OneDrive/Desktop/JTP-PROJECT%20ROUND/documentation/FRONTEND_ARCHITECTURE.md) for the page-by-page breakdown.
...
Every request to a product route passes through the auth dependency and rate limiter *before* reaching business logic - see [THIRD_PARTY_API.md](file:///c:/Users/SHRIYASH%20SAWANT/OneDrive/Desktop/JTP-PROJECT%20ROUND/documentation/THIRD_PARTY_API.md) for the two-tier key model this enables. `GET /api/v1/health` is the one route that bypasses `AuthDep` entirely (standard liveness-probe convention).
...
The React application is located under the `frontend/` directory. It uses Next.js 14 App Router patterns.
*   `frontend/src/app/page.tsx`: The primary landing page component containing the conversational search interface. Handles basic state initialization and input validation.
*   `frontend/src/app/search/page.tsx`: The detailed search results page. Features multi-parameter query filtering (scenarios, scent families, skin types, budgets, longevity, and sillage preferences) and manages local state replication via `sessionStorage` to allow back-navigation without losing search parameters.
*   `frontend/src/app/perfume/[id]/page.tsx`: The detail visualization page for a selected perfume. Displays metadata, launch years, country of origin, scent pyramid tiers, and provides navigation triggers to the duplicate engine.
*   `frontend/src/app/dupe/page.tsx`: The olfactory duplicate finder interface. Allows users to query a benchmark scent and retrieve budget-constrained alternatives. It is integrated with URL query parameter parsing to allow instant search execution.
*   `frontend/src/lib/api.ts`: Centralizes type contracts (`Perfume`, `SearchResponse`, `PerfumeDetail`) and HTTP fetch abstractions.
...
*   `backend/app/services/decision_engine.py`: The core of the matching algorithm. Executes score calculations, scenario mappings, chemical bridge checks, concentration performance updates, and unisex gender bias modifiers - see [DECISION_ENGINE.md](file:///c:/Users/SHRIYASH%20SAWANT/OneDrive/Desktop/JTP-PROJECT%20ROUND/documentation/DECISION_ENGINE.md).
...
*   `backend/app/services/rate_limiter.py`: In-memory token-bucket rate limiter keyed per API key (and per-client-IP for publishable keys specifically) - see [THIRD_PARTY_API.md](file:///c:/Users/SHRIYASH%20SAWANT/OneDrive/Desktop/JTP-PROJECT%20ROUND/documentation/THIRD_PARTY_API.md).
...
**Data ingestion and schema evolution** (see [DATA_INGESTION_PIPELINE.md](file:///c:/Users/SHRIYASH%20SAWANT/OneDrive/Desktop/JTP-PROJECT%20ROUND/documentation/DATA_INGESTION_PIPELINE.md) for the full rationale):
...
| 2 | Roadmap | A `/metrics` Prometheus endpoint (`prometheus_client`) - latency histograms, error rates, DB pool utilization | Small, standard, and the actual prerequisite for ever justifying a caching layer later - real numbers are needed before knowing whether there's a latency problem worth caching for. |
...
| 5 | Roadmap | Admin/ops trust boundary - `/api/v1/admin/*`, role-gated, audit log | Supports reviewing scraped/ingested data and approving changes once ingestion volume makes that a real human workflow, not a hypothetical one. |
